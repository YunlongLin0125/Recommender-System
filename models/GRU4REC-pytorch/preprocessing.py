...
dataBefore = 'C:\\Users\\Yunlong\\Desktop\\Recommender-System\\datasets\\ml-1m\\ratings.dat'  # Path to Original
# Training Dataset "Clicks" File
dataTestBefore = 'C:\\Users\\Yunlong\\Desktop\\Recommender-System\\datasets\\ml-1m\\ratings.dat'  # Path to Original
# Testing Dataset "Clicks" File
dataAfter = 'C:\\Users\\Yunlong\\Desktop\\Recommender-System\\models\\GRU4REC-pytorch\\data\\processed\\'  #
...
train = pd.read_csv(dataBefore, sep='::', header=None, usecols=[0, 1, 2, 3], dtype={0: np.int32, 1: str, 2: np.int64},
                    engine='python')
test = pd.read_csv(dataTestBefore, sep='::', header=None, usecols=[0, 1, 2, 3],
                   dtype={0: np.int32, 1: str, 2: np.int64}, engine='python')
train.columns = ['SessionID', 'ItemID', 'Rating', 'Time']  # Headers of dataframe
test.columns = ['SessionID', 'ItemID', 'Rating', 'Time']  # Headers of dataframe
...
# test['Time'] = test['Time'].apply(lambda x: datetime.datetime.fromtimestamp(x))
# # train['Time'] = train.Time.apply(
#     lambda x: datetime.datetime.strptime(x, '%Y-%m-%dT%H:%M:%S.%fZ').timestamp())  # Convert time objects to timestamp
# test['Time'] = test.Time.apply(
#     lambda x: datetime.datetime.strptime(x, '%Y-%m-%dT%H:%M:%S.%fZ').timestamp())  # Convert time objects to timestamp
...
print('Testing Set has', len(test), 'Events, ', test.SessionID.nunique(), 'Sessions, and', test.ItemID.nunique(),
      'Items\n\n')
test.to_csv(dataAfter + 'ml-1m_Test.txt', sep=',', index=False)
...
sessionValid = sessionMaxTime[
    sessionMaxTime >= (timeMax - dayTime)].index  # validation split is all sessions that ended during the last 2nd day
...
trainTR.to_csv(dataAfter + 'ml-1m_TrainOnly.txt', sep=',', index=False)
print('Validation Set has', len(trainVD), 'Events, ', trainVD.SessionID.nunique(), 'Sessions, and',
      trainVD.ItemID.nunique(), 'Items\n\n')
trainVD.to_csv(dataAfter + 'ml-1m_Valid.txt', sep=',', index=False)
